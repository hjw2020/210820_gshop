<template>
  <section class="loginContainer">
            ...
            <div :class="{on: !loginMode}">
              <section>
                ...
                <section class="login_message">
                  ...
                  <img class="get_verification" ref="captcha" @click="changeCaptcha" src="http://localhost:4000/captcha" alt="captcha">
                </section>
              </section>
            </div>
    ...
    <AlertTip :alertText="alertText" v-show="isShowAlert" @closeTip="isShowAlert = false"></AlertTip>
  </section>
</template>
...
export default {
  ...
  data () {
    return {
      loginMode: true,  // true代表短信登录，false代表密码登录
      phone: '',  // 手机号
      computeTime: 0,  // 计时的时间
  ...
  methods: {
    ...
    async login () {
      ...
      let result
      // 前台表单验证
      if (this.loginMode) { // 短信登录
        const {rightPhone, phone, code} = this;
        if (!rightPhone) {
          // 手机号输入有误，请重新输入
          this.showAlert('手机号输入有误，请重新输入')
          return
        } else if (!(/^\d{6}/.test(code))) {
          // 短信验证码有误，请重新输入
          this.showAlert('短信验证码有误，请重新输入')
          return
        }
        // 发送ajax请求登录
